                                                  ...
                                                  FlMethodCall* method_call)
{
    ...
    else if (strcmp(method, "getRenderContext") == 0)
    {
        struct utsname uname_data = {};
        uname(&uname_data);
        g_autofree gchar* version = g_strdup_printf("%p", nullptr);
        g_autoptr(FlValue) resultString = fl_value_new_string(version);
        response =
            FL_METHOD_RESPONSE(fl_method_success_response_new(resultString));
    }
    else
    {
